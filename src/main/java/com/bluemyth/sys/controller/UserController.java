...
public class UserController extends SuperController {
    ...
    public RestStatus findById(@PathVariable String id) {
        User entity = thisService.selectById(id);
        return RestStatusUtil.success(entity);
    }

    @ApiOperation(value = "获取信息->依条件", notes = "查询条件,所有条件均以and连接(非必填)")
    @GetMapping("/find")
    public RestStatus find(User entity) {
        List<User> list = thisService.selectList(new EntityWrapper<User>(entity));
        return RestStatusUtil.success(list);
    }

    @ApiOperation(value = "新增信息", notes = "不传主键id")
    @PostMapping
    public RestStatus add(User entity) {
    ...
    @PutMapping
    public RestStatus update(User entity) {
